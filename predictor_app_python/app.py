...
f.write("SNR,MCS,PRBs,TBS,m_type,predicted_dec_time,dec_time,p_err\n")

def estimate_service_time(SNR, MCS, PRBs, TBS):
    # Run LPU Models
    t_cpu, e_cpu, t_gpu, e_gpu = lpu_models.estimate_service_time(SNR, MCS, PRBs, TBS)
    ...
    for cpu_val in cpu_tmp_df.Total_latency_us.values:
        ...
        f.write(f"{SNR},{MCS},{PRBs},{TBS},CPU,{t_cpu},{cpu_val},{err}\n")
    ...
    for gpu_val in gpu_tmp_df.Total_latency_us.values:
        ...
        # Dump results into file
        f.write(f"{SNR},{MCS},{PRBs},{TBS},GPU,{t_gpu},{gpu_val},{err}\n")
